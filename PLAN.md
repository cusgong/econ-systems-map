...
## 지금 단계
v2.4.0 배포: https://econ-systems-map.vercel.app (UX 전면 개보수: AI 채팅 견고화·온보딩 개편·모바일 인체공학·a11y·아이덴티티 자산). "지금" 자동화 파이프라인은 시크릿 **3/6 등록 완료**(VERCEL_TOKEN·ORG_ID·PROJECT_ID). **활성화 대기: Marcus 키 3개(ECOS·FRED·setup-token)를 api-keys.local.env에 추가 → 나머지 시크릿 3개 등록 → 첫 workflow_dispatch 검증.**
