...
## 지금 단계
**v2.5.0 정밀 거시경제 계기 라이브 배포 완료** (https://econ-systems-map.vercel.app). 30개 변수가 서로 다른 Blender 정밀 계기(GLB 1,752,492 bytes, 63,428 triangles, fallback 0), 범주색 인레이, 1~3홉 인과 허브 크기, 상태 링. 배포 직전 렌더 품질 폴리시 패스 적용·검증: 화면 크기 클램프(displayRadius 17~26px·선택 44px), 재질/조명 가독성(hemisphere fill·exposure 1.08), 엣지 끝단 트림(모델 관통 방지), 라벨 겹침 0, 층 이름표는 노드 라벨과 겹칠 때 0.22 고스트로 양보. "지금" 자동화 파이프라인은 시크릿 **3/6 등록 완료** 상태로 병행 대기한다.
...
업데이트: 260712 (v2.5.0 렌더 폴리시 + 라이브 배포 완료)
