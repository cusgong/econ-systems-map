...
## 지금 단계
**v2.7.0 계기 입체화 — 라이브 배포 완료** (https://econ-systems-map.vercel.app, 260713). Marcus 후속 지적 "기준금리 같은 요소가 너무 평면적"의 근본 해결. 라이브 검증: main.js `v2.7.0`, GLB 1,801,240 bytes(로컬 커밋본과 byte 동일). 배포는 Marcus가 PowerShell에서 토큰 파일 로드 후 `npx vercel deploy --prod`로 실행(세션 권한 분류기가 에이전트 직접 배포를 차단하는 건 v2.6.0과 동일). v2.6.0 런타임 시각 개편(헤일로·빌보드 링·범주색)은 사이드 뷰 존재감을 올렸지만 **모델 지오메트리 자체가 평면**인 문제는 남아 있었다 → Blender 정본을 재저작해 납작하던 18개 계기(기준금리 0.24→0.62 포함)의 최소/최대 축 비율을 전부 ≥0.50으로 끌어올림. 기준금리(policy_rate)는 얇은 다이얼 링에 게이지 캔(케이스)+입체 컬러 베젤 링을 붙여 실제 계기로, 나머지는 얇은 축을 깊게(섀시·배럴·헤더 드럼) 또는 최장축을 줄여 입체화. 전 모델 삼각형 <3000 하드캡·accent 면적비 0.10~0.20·실루엣 IoU·베벨 최소치 유지. 프로즌 해시/accent tuple/삼각형 밴드 전면 재생성, 결정론 검증(전체 체인 2회 byte 동일 GLB, 변경 없는 12개 모델 해시 git HEAD와 일치). GLB 1.75MB→1.80MB. "지금" 자동화 파이프라인은 시크릿 **3/6 등록 완료** 상태로 병행 대기한다.
...
업데이트: 260713 (v2.7.0 계기 입체화 — 평면 계기 18개 재저작)
