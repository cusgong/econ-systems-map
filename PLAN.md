...
## 지금 단계
**v2.7.0 계기 입체화 — GitHub push 완료(origin/main=799f4d9), Vercel prod 배포 대기** (이 세션 권한 분류기가 `npx vercel deploy`를 차단 → v2.6.0과 동일하게 배포 명령 1회 직접 실행 필요. GitHub 자동배포는 미연결이라 push만으로는 라이브 미갱신). Marcus 후속 지적 "기준금리 같은 요소가 너무 평면적"의 근본 해결. v2.6.0 런타임 시각 개편(헤일로·빌보드 링·범주색)은 사이드 뷰 존재감을 올렸지만 **모델 지오메트리 자체가 평면**인 문제는 남아 있었다 → Blender 정본을 재저작해 납작하던 18개 계기(기준금리 0.24→0.62 포함)의 최소/최대 축 비율을 전부 ≥0.50으로 끌어올림. 기준금리(policy_rate)는 얇은 다이얼 링에 게이지 캔(케이스)+입체 컬러 베젤 링을 붙여 실제 계기로, 나머지는 얇은 축을 깊게(섀시·배럴·헤더 드럼) 또는 최장축을 줄여 입체화. 전 모델 삼각형 <3000 하드캡·accent 면적비 0.10~0.20·실루엣 IoU·베벨 최소치 유지. 프로즌 해시/accent tuple/삼각형 밴드 전면 재생성, 결정론 검증(전체 체인 2회 byte 동일 GLB, 변경 없는 12개 모델 해시 git HEAD와 일치). GLB 1.75MB→1.80MB. "지금" 자동화 파이프라인은 시크릿 **3/6 등록 완료** 상태로 병행 대기한다.
