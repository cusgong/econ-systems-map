...
## 지금 단계
**v2.8.0 실루엣 균형 입체화 + 패널 계기 뷰어 — 배포 대기** (Marcus 후속: "입체화가 충분치 않다, 한 perspective에서만 분명하다 + 클릭한 모델 자동회전으로 자세히 보기"). ①진단: AABB 축비가 잘못된 지표였음 — 실루엣 점유로 보니 ~18개가 정면에만 질량이 몰려 옆면이 빈약(tech 0.405, cpi 0.434, fx 0.436…). ②올바른 지표(3면 점유 균형 occ_ratio ≥0.62)로 재저작: 약한 면에 실제 질량(사이드 포드·풀 배럴·스팬 브래킷) 부여, 캡 근처(fed_rate·tech) 정면 디테일 재분배 → 전 18개 0.635~0.905 달성. ③패널 계기 뷰어 신설(`js/model-viewer.js`): 노드 선택 시 그 계기 크게·자동 회전·드래그 인스펙트. GLB 1.80→1.87MB. 검증: node 77/77, node-specs 4/4, 결정론 재증명(byte 동일 GLB, 12개 불변 모델 해시 v2.7.0과 일치), CDP로 tech/fx가 입체 오브젝트로 렌더 확인. 배포는 v2.7.0과 동일하게 Marcus가 PowerShell 토큰 로드 후 실행.
...
업데이트: 260713 (v2.8.0 실루엣 균형 입체화 + 패널 자동회전 뷰어)
