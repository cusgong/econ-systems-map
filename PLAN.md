...
## 지금 단계
v2.3.0 커밋·push 완료(라이브는 v2.2.0). "지금" 보드 무인 자동 업데이트 파이프라인 구축: 수치 매일(ECOS 13+FRED 2) + 해설 주간(Claude 에이전트, 검증 게이트). **활성화 대기: Marcus 키 3개(ECOS·FRED·setup-token) + GH 시크릿 6개 등록 + 첫 workflow_dispatch 검증.**
...
- [ ] (AI, 키 도착 후) GH 시크릿 6개 등록 → workflow_dispatch 첫 실행 → 라이브 v2.3.0 검증
