...
## 지금 단계
**v2.10.0 판독성 정밀 다듬기 — 배포 대기** (Marcus: "또렷하고 명확하게 다듬을 수 있는 거 다 다듬어 줘"). 30개 전체를 재판정해 애매한 13개를 2차 정밀 수정: ①환율=자이로 폐기→동전+순환 화살표(만국 공용 환전 글리프, rotate-y로 화살표가 동전을 돎) ②부동산=포털 프레임 폐기→박공지붕 집(범주색 지붕·굴뚝·현관·창) ③미국금리=얕은 퍽 다이얼+크게 돌출된 별+바늘(3/4에서 별 즉독) ④재정지출=주랑+페디먼트+돔 의사당(와이드-로우 프로파일로 집과 실루엣 분리)+코인 디스크 3개 ⑤투자심리=열린 아치 진자(전면 개방 104°, 20° 스윙 정지, 공포-탐욕 눈금판) ⑥소비=테이퍼 종이백+접힌 커프+굵은 손잡이 ⑦고용=또렷한 3인(큰 머리·목 간격·네임플레이트) ⑧기준금리=바늘을 베젤 링 앞으로 돌출+40% 확대(링 속에 파묻혀 안 보이던 문제) ⑨신용스프레드=굵은 양방향 화살표가 두 바 안쪽면을 눌러 잇기 ⑩자본유입=깔때기 위 큰 셰브론 3개 ⑪경상수지=접시 1.5×·불균형 강조 ⑫주가=키 큰 캔들+굵은 심지 ⑬소비자심리=넓은 아치 눈금+긴 바늘. 중간 실패 2건 직접 수정: consumer_conf 액센트 thin-Y 계약 위반(0.361>0.305)→Y 스팬 압축, housing/fiscal 실루엣 IoU 0.80+ 충돌→fiscal 와이드-로우 재비례. GLB 1.71→1.69MB. 검증: node 77/77·specs 4/4·파이프라인, 결정론 재증명(byte 동일), CDP 13개 판독 확인. 배포는 Marcus가 PowerShell 토큰 로드 후 실행.

_v2.7.0(라이브 260713, econ-systems-map.vercel.app): 계기 18개 AABB 축비 ≥0.50 1차 입체화._ v2.6.0 런타임 시각 개편(헤일로·빌보드 링·범주색)은 사이드 뷰 존재감을 올렸지만 **모델 지오메트리 자체가 평면**인 문제는 남아 있었다 → Blender 정본을 재저작해 납작하던 18개 계기(기준금리 0.24→0.62 포함)의 최소/최대 축 비율을 전부 ≥0.50으로 끌어올림. 기준금리(policy_rate)는 얇은 다이얼 링에 게이지 캔(케이스)+입체 컬러 베젤 링을 붙여 실제 계기로, 나머지는 얇은 축을 깊게(섀시·배럴·헤더 드럼) 또는 최장축을 줄여 입체화. 전 모델 삼각형 <3000 하드캡·accent 면적비 0.10~0.20·실루엣 IoU·베벨 최소치 유지. 프로즌 해시/accent tuple/삼각형 밴드 전면 재생성, 결정론 검증(전체 체인 2회 byte 동일 GLB, 변경 없는 12개 모델 해시 git HEAD와 일치). GLB 1.75MB→1.80MB. "지금" 자동화 파이프라인은 시크릿 **3/6 등록 완료** 상태로 병행 대기한다.
...
## 최근 완료
- v2.10.0 판독성 정밀 다듬기 (Marcus: "또렷하고 명확하게 다듬을 수 있는 거 다"): 30개 재판정→13개 2차 수정(위 '지금 단계' 상세). 7 에이전트 병렬 + policy_rate·중간실패 2건은 직접 수정. 계약 전면 재생성(tuple 13·밴드·베벨 실측 복원 - housing accent 8로 하향·proof SHA·30 해시), 게이트 ON 체인 byte 동일, 누적 삼각형 proof 13956/12모델 26836/전체 65220 전부 범위 내.
- v2.9.0 아이콘형 모델 재설계 (Marcus: "뭘 의미하는지 모르겠는 모델들 하나하나 체크해서 개선"): ①진단: 30개를 패널 뷰어로 CDP 렌더 → 정밀 계기 DNA가 아름답지만 추상적이라 ~19개가 개념 전달 실패(임금·GDP·물가·고용 등). Marcus에게 방향 3안 제시(아이콘형/엠블럼각인/은유강화) → 아이콘형 선택. ②재설계: 5개 빌더 파일 병렬 에이전트, 20개를 개념이 즉독되는 상징으로(어두운 기계 재질·범주색 유지, 본체=구조/액센트=범주색 핵심): 금고+코인(은행대출)·장바구니(물가)·온도계(기대인플레)·계단코인(임금)·수출화살표·저울(경상수지)·깔때기(자본유입)·지구본화살표(글로벌경기)·작업자(고용)·상승막대(기업실적)·부러진사슬(부도)·상승화살표(GDP)·족쇄(가계부채)·의사당동전(재정지출)·무드미터(소비자심리)·상승라인(시장금리)·갭캘리퍼(신용스프레드)·별다이얼(미국금리)·쇼핑백(소비)·캔들(주가). 전부 2메시·<3000삼각형·accent 0.10~0.20·occ_ratio·thin-axis·pivot라벨 계약 유지, IoU 통과. ③2차 수정: 판독 렌더에서 안 읽히던 market_rate(상승라인이 본체색이라 안 보임)를 라인을 범주색 액센트로, defaults(초록 별=부도와 반대 의미)를 하강 화살표로. ④재생성: accent tuple 20개·삼각형 밴드·베벨 최소치(비움→실측 복원)·proof SHA·12모델밴드·30 루트해시 전면 갱신, 전체 체인 해시 게이트 ON 재실행 byte 동일 GLB. GLB 1.87→1.71MB. 검증: node 77/77, node-specs 4/4, Blender 파이프라인(백그라운드), CDP로 20개 아이콘 판독 확인. 판독되던 11개는 유지.
- v2.8.0 실루엣 균형 입체화 + 패널 계기 뷰어 (Marcus: "입체화가 충분치 않다, 한 perspective에서만 분명하다 + 클릭한 모델 자동회전으로 보기"): ①패널 뷰어(`js/model-viewer.js`): 독립 GLTFLoader로 GLB 1회 로드, 선택 노드 루트 클론, RoomEnvironment+키/림 라이트, OrbitControls autoRotate(드래그 시 정지·2.6초 후 재개, reduced-motion off), 바디 다크티타늄/accent 범주색 emissive 0.5, 캔버스 재부모화로 패널 재렌더에도 회전 유지, isConnected 게이트로 미표시 시 렌더 스킵. ui.js 노드 상세 상단에 삽입(rAF 후 attach), main.js에서 생성해 deps 주입, `.model-viewer` CSS(224px·라디얼글로우·⟲힌트), EN 문자열 추가. 여러 모델 CDP 콘솔 에러 0·회전 프레임차 검증. ②진단: 검증기가 이미 산출하는 front/side/top 실루엣 점유로 "한 각도에서만 분명한" 모델 랭킹 → AABB 축비 ≥0.5여도 정면 질량 집중이면 occ_ratio 낮음(tech 0.405 등 ~18개). 하네스에 occ_ratio(점유 min/max) 측정 추가. ③재저작: 5개 빌더 파일 병렬 에이전트, 목표 occ_ratio ≥0.62 — 약한 면(주로 옆 Y-Z)에 실제 질량 부여(cpi 드럼 배럴 심화, bank_lending 아웃보드 펌프 하우징 치크, fx 자이로 링/로터, inflation_exp 와이드 섀시 등), 캡 근처(fed_rate 2960·tech 2760)는 정면 재분배. 전 18개 occ 0.635~0.905·AABB 0.589~0.873·삼각형 <3000·accent 0.10~0.20·thin-axis 유지, 정체성 보존. ④재생성: accent tuple 18개·삼각형 밴드(validate+test 3딕트)·proof 총합밴드(13000→14200)·12모델밴드·proof SHA·30 루트해시 전면 갱신, 전체 체인 해시 게이트 ON 재실행 byte 동일 GLB. 검증: node 77/77, node-specs 4/4, Blender 파이프라인(백그라운드), CDP로 tech/fx 입체 렌더 확인.
- v2.7.0 계기 입체화 (Marcus: "기준금리 같은 요소가 너무 평면적이라는거야"): v2.6.0 런타임 개편으로도 안 풀린 **지오메트리 평면성**을 Blender 재저작으로 근본 해결. ①측정: 30개 GLB 모델의 min/max AABB 축비를 측정해 <0.50인 18개를 확정(최악 consumer_conf 0.222, 기준금리 0.240). ②하네스: 검증기 `_evaluated_mesh_metrics`를 그대로 재사용하는 단일-모델 고속 프로브(`_measure_model.py`/`_measure_from_blend.py`)를 만들어 전체 8단계 체인 없이 축비·삼각형·accent비·accent tuple을 초 단위로 측정 → 빌더 반복 튜닝을 실용화. ③튜닝: 5개 빌더 파일을 병렬 에이전트로(각 자기 파일만), scaffold의 policy_rate는 수작업으로 — 얇은 축을 깊게(게이지 캔·섀시·배럴·헤더 드럼·armor 두께) 또는 최장축 축소로 전 18개를 ≥0.50에 안착(0.538~0.619), 전부 삼각형 <3000 하드캡·accent 면적비 0.10~0.20·thin-axis 계약·실루엣 유지, 정체성(전면 실루엣) 보존. ④재생성: ROOT_CONTRACT_HASHES를 비워 프로즌 게이트를 통과시키고(자기참조 우회), accent tuple 18개·삼각형 밴드(validate+test 3곳 미러)·policy 바디밴드를 실측값으로 갱신, 전체 체인 실행→30 루트 해시 스냅샷→재기록→해시 게이트 ON으로 재실행 그린. ⑤결정론 증명: 전체 체인 2회가 byte 동일 GLB(1,801,240) 산출, 변경 없는 12개 모델 해시가 git HEAD와 정확히 일치(환경 결정성 확인). ⑥검증: node 77/77, Blender 파이프라인 35 계약, proof GLB SHA·삼각형 밴드 재계산, CDP 실렌더 사이드/근접/선택 뷰로 기준금리가 실제 입체 게이지로 렌더됨 확인.
...
업데이트: 260714 (v2.10.0 판독성 정밀 다듬기 — 13개 2차 수정)
