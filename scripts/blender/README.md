...
The integration suite covers the RED fixture, exact six-ID proof scope, per-model and total triangle bands, accent area, normalized bounds, 12 primitives, real weighted three-segment bevels, fractional-weight rejection, CPU-rasterized three-view silhouette separation, canonical one-owner mesh data, accent motion/pivot drift, decoded GLB geometry-payload drift, deterministic GLB export, and preservation of a sentinel output when an invalid export is attempted.
