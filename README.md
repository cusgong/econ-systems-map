...
v2.6.0은 계기의 시각 존재감을 개편했다. 노드마다 범주색 가산 헤일로(림 글로우)가 각도 불변 실루엣을 만들고, 계기는 유휴 자전·미세 장동으로 모든 실루엣을 보여주며(모션 축소 시 정지), 오버레이 링 4종은 카메라를 향해 빌보드된다. accent 인레이 emissive 부스트와 바디 범주 틴트로 범주색이 지도 스케일에서 읽히고, 팔레트 4색은 색각이상 분리 기준으로 재조정되었다(검증 기록: DESIGN.md §7).

정식 자산은 30 IDs, 60 primitives, 63,428 triangles, 1,752,492 bytes이며 Blender 계약 35개와 Node.js 회귀 테스트를 통과한다(v2.6.0 기준 77개). 107개 passive edge는 현재 데이터에서 3개 배치로 렌더되고, contested edge의 점선 위상은 엣지별로 독립 누적된다. 헤일로 스프라이트는 노드당 1 드로콜(총 +30)을 추가한다.
