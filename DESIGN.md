...
- **씬 안무(three.js Clock 절차 애니메이션이 본체)**:
  ...
  - **앰비언트**: 글로우 브리딩 `1+sin(t×1.4)×0.05`, autoRotate 0.35, 흐름 파티클 speed `0.05+0.035×strength`.
...
- 정책 `#35d0b0` · 통화·금융 `#4fd8ff` · 자산시장 `#9d8cff` · 시장 심리 `#ff9ecb` · 실물경제 `#6fe38a` · 물가·임금 `#ff7666` · 원자재 `#ffc857` · 외생 변수 `#c678ff` · 대외·환율 `#5f8dff`
- 노드 구·와이어프레임·글로우·라벨 도트·범례·검색 도트가 전부 이 9색에서 파생. 부호색(§1-2)과 역할이 겹치지 않게 유지할 것(범주 = 정체성, 부호 = 관계).
- **검증**: `validate_palette.js` 미실행 (260712 레트로핏 시점 실행 이력 없음 — 팔레트 변경 시 실행하고 여기 기록).
...
- **리터럴로 남은 범위**:
  ...
  - `css/lang-toggle.css`: 활성 배경 `#2563eb` — **i18n 키트 기본 블루가 미치환**으로 남아 서비스 시안과 어긋난다(알려진 드리프트; 고치려면 `--lt-active-bg`를 `--cy` 계열로 오버라이드).
