...
- 260714: 판독성 2차 정밀화(v2.10.0). 애매하게 남은 13개를 재수정: 환율=동전+순환 화살표(자이로 폐기), 부동산=박공지붕 집(포털 프레임 폐기), 기준금리 바늘을 베젤 링 앞으로 돌출 등. 교훈 2건 병기 — (1) 액센트 thin-axis 계약은 "Y ≤ 0.45×min(X,Z)"로 계산해 검증할 것(전 축 대비 아님), (2) 새 아이콘 도입 시 기존 모델과의 3면 실루엣 IoU 충돌을 상정하고 비례(와이드-로우 vs 톨-내로)를 처음부터 분리할 것(집 vs 의사당 충돌 사례).
- 260713: 노드 모델 판독성 규칙(v2.9.0). 정밀 계기 DNA는 아름답지만 추상적이라 20개 모델이 개념을 전달하지 못했다 → **아이콘 우선**으로 조정: 각 노드 모델은 그 경제 개념을 즉독 가능한 상징으로 읽혀야 한다(어두운 기계 재질·범주색·챔퍼는 유지해 패밀리 통일). 본체=어두운 구조, 액센트=범주색으로 개념의 핵심(코인·화살표·저울추 등)을 담는다. 판독성은 패널 뷰어 CDP 렌더로 검증(하네스는 기하만 판정하므로 사람이 눈으로 확인). 판독되던 11개(기준금리 게이지·유가 밸브·원자재 광석통 등)는 유지.
- 260713: 계기 입체성 규칙 추가(v2.7.0). 30개 노드 계기(정본 `scripts/blender/`, 파생 `data/models/econ-node-library.glb`)는 min/max AABB 축비 ≥0.50을 지켜 어느 각도에서도 납작한 판이 아니라 계기로 읽혀야 한다. 정체성(전면 실루엣) 보존, 삼각형 <3000/모델, accent 면적비 0.10~0.20이 동반 계약. 재저작 시 `_measure_model.py` 프로브로 검증하고 프로즌 해시·accent tuple·삼각형 밴드를 재생성한다.
- 260713: 입체성 지표를 실루엣 균형으로 격상 + 패널 뷰어 추가(v2.8.0). ①AABB 축비만으로는 부족(정면에 질량이 몰리면 옆 실루엣이 빈약해도 축비는 높음). 진짜 지표 = **3면(front/side/top) 실루엣 점유 균형 occ_ratio = min/max ≥0.62**. `_measure_model.py`가 검증기 `_projected_occupancy_mask`로 이를 산출한다. 약한 면(주로 옆면 Y-Z)에는 얇은 스파인이 아니라 그 면을 채우는 실제 질량(사이드 포드·풀 배럴·스팬 브래킷)을 준다. 삼각형 캡 근처 모델은 정면 과잉 테셀레이션을 약한 면으로 재분배. ②패널 계기 뷰어(`js/model-viewer.js`): 노드 선택 시 그 계기를 크게·조명·자동 회전으로 보여준다(드래그 인스펙트, 놓으면 재개, reduced-motion 정지). GLB 1회 로드 후 선택 루트 클론, 바디=다크 티타늄/accent=범주색 emissive. 지도는 한 면만 보여주므로 뷰어가 전 각도 확인을 맡는다.
